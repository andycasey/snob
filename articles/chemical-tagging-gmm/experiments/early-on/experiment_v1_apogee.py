...
predictors = ("FE_H",  "AL_FE", "NA_FE", "O_FE", "CA_FE", "MG_FE", )
...
for n in range(1, 1 + N):
    ...
    for m in range(M):
        ...
        match = finite \
              * np.in1d(catalog["FIELD"], selected_cluster_names) \
              * (catalog["ASSOCIATION_PROB"] >= min_membership_probability)

        for cluster_name in selected_cluster_names:
            _ = catalog["FIELD"][match] == cluster_name
            idx = np.random.choice(range(_.sum()), size=min_stars_per_cluster, replace=False)
            
            mi = np.where(match)[0][np.where(_)[0]]
            match[mi] = False
            match[mi[idx]] = True

            #match[m][_]
            #match[_] = False
            #match[np.where(_)[0][idx]] = True
        ...
        model = snob.GaussianMixture(covariance_regularization=1e-10,
            covariance_type=covariance_type)
...
with open("v1-apogee-results-{}-{}.pkl".format(covariance_type,

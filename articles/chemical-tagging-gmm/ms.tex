...
\begin{equation}
I(\weights) = \frac{K - 1}{2}\log{N} - \frac{1}{2}\sum_{k=1}^{K}\weight_k - \log{\Gamma{\left(K\right)}} \quad ,
\end{equation}
...
derivatives of the negative log-likelihood $-\likelihood(\data|\vecmean,\veccov)$.
This is difficult to calculate for mixtures of Gaussians, and for this reason
we approximate the determinant of the Fisher information 
$|\mathcal{F}(\vecmean, \veccov)|$ as the product of 
$|\mathcal{F}\left(\vecmean\right)|$ and $|\mathcal{F}\left(\veccov\right)|$ 
...
\begin{eqnarray}
  |\mathcal{F}(\vecmean)| = \left|-\frac{\delta^2\likelihood}{\delta\vecmean^2}\right| = N^d|\veccov|^{-1} \quad .
\end{eqnarray}
...
\subsection{Encoding the message}

Given our priors, likelihood function, and approximate expressions for the
Fisher information, we are now in a position to calculate the total message
length.
There are a number of components that go into the message:




A common method to estimate the parameters of a Gaussian distribution is by
maximum likelihood, where the log-likelihood $\likelihood$ is maximized. In

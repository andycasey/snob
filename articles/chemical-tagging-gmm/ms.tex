...
\begin{equation}
  I(\vectheta,\data) 
    = \frac{p}{2}\log{q_p} - \log{\left(\frac{h(\vectheta)}{\sqrt{|\mathcal{F}(\vectheta)|}}\right)}
      - \mathcal{L}\left(\data|\vectheta\right) + \frac{p}{2}
\end{equation}


\noindent{}is the MML estimate.
Here $h(\vectheta)$ is the prior on the hypothesis, $p$ is the number of free
parameters in the model, and $|\mathcal{F}\left(\vectheta\right)|$ is the
determinant of the \emph{expected} Fisher information matrix (the second-order
partial derivatives of the negative log-likelihood function $-\mathcal{L}(\data|\vectheta)$.
...
Unfortunately, strict MML is generally intractable, and therefore we require
an app

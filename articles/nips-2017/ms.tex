...
\title{
  ...
  using variational Bayesian methods
}
...
\begin{abstract}
  The number of components in a Gaussian mixture model is usually determined
  by applying a chosen penalty heuristic.
  This approach requires optimizing a set of mixtures of increasing complexity,
  and unnecessarily separates model selection from parameter estimation.
  
  We derive an objective function for a finite Gaussian mixture model using
  Minimum Message Length (MML), and derive an expression for future mixtures
  that have yet to be observed.


  Although future mixtures have yet to be trialled, we show that we can
  calculate probability distributions for those future mixtures, allowing 
  for a \emph{jumping} search algorithm that simultaneously performs model
  selection and parameter estimation for Gaussian mixtures in linear time.
